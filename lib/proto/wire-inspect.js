...
// How deep to descend before giving up. The old limit of 3 was not actually hit
// by anything in issue #61 - weeklyProfile, the deepest, nests exactly three and
// so just fit. The limit still needed raising, because the old one reported a
// cut as an ordinary `len(n)` field, which is indistinguishable from a payload
// genuinely too opaque to read: sitting one level under the limit, nothing said
// so. Both are fixed - deeper, and a cut now says it is a cut.
...
function formatNode(node) {
  switch (node.kind) {
    ...
    case 'packed':
      // The hex comes too. A run of bytes that is not a message and not text
      // still parses as varints far more often than it means them, and this is
      // precisely the spot where over-reading the bytes is the failure mode -
      // weeklyProfile's field 4 lands here.
      return `${node.field}:packed[${node.values.join(',')}]=0x${hexPreview(node.bytes)}`;
...
const SCALAR_FOR_KIND = {
  ...
  fixed32: 'float',
  string: 'string',
  // Not `repeated int64`. Bytes that are neither a message nor text parse as
  // varints far more often than they mean them, so a draft that declared a
  // packed field would be making exactly the confident guess this whole module
  // exists to stop making. The varint reading goes in a comment instead.
  packed: 'bytes',
  bytes: 'bytes',
  empty: 'bytes',
  'depth-limit': 'bytes',
...
function sampleOf(node) {
  // The signed reading for anything in the negative range: a draft that says
  // `observed 18446744073709551615` invites the same misreading the issue made,
  // where -1 looked like a sentinel value.
  if (node.kind === 'varint') return String(node.unsigned >= U64_SIGN_BIT ? node.signed : node.unsigned);
  ...
  if (node.kind === 'packed') return `0x${node.bytes.toString('hex')}`;
  return null;
}
...
function emitMessage(name, instances, defs) {
  ...
  for (const entry of mergeShape(instances)) {
    ...
    if (kind === 'packed') {
      notes.push('UNKNOWN: not a message and not text; parses as packed varints but may be an opaque blob');
    }

    const type = SCALAR_FOR_KIND[kind] || 'bytes';
    lines.push(`  ${repeated}${type} f${entry.field} = ${entry.field};${notes.length ? `  // ${notes.join('; ')}` : ''}`);
  }
