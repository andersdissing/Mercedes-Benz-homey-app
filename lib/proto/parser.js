...
class ProtoParser {
   ...
   * shape needed to declare it.
   *
   * @param {Buffer} bytes - raw VehicleAttributeStatus
   * @param {Number} depth - recursion guard
   * @returns {String}
   */
  _describeRawAttribute(bytes, depth = 0) {
    const parts = [];
      ...
      while (reader.pos < reader.len) {
        ...
        switch (wireType) {
          ...
          case 2: {
            ...
            const nested = depth < 3 ? this._describeRawAttribute(sub, depth + 1) : null;
          ...
          case 5:
            ...
            break;
          default:
            // Unknown wire type - the rest of the buffer can't be trusted.
            parts.push(`${field}:?wire${wireType}`);
            return parts.join(' ');
        }
      }
    } catch (err) {
      // A length-delimited field holding a plain string parses as garbage rather
      // than throwing every time, so a partial description is the normal outcome
      // here and is still worth printing.
