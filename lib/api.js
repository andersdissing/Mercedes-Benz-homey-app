...
class MercedesAPI {
   ...
   *
   * Consecutive refusals double the window: a fixed one is a closed loop that
   * pokes the limiter at a steady rate and never gives it the quiet period it
   * is waiting for.
   */
  _registerRestRateLimit() {
    this._restRateLimitStrikes++;
    const window = Math.min(
      this.REST_RATE_LIMIT_BACKOFF * Math.pow(2, this._restRateLimitStrikes - 1),
      this.MAX_REST_RATE_LIMIT_BACKOFF,
    );
    this._restBlockedUntil = Date.now() + window;

    this.homey.app.error(
      `[API] REST endpoints rate-limited (HTTP 429) - pausing polls for ${Math.round(window / 60000)} min `
      + `(consecutive refusals: ${this._restRateLimitStrikes})`,
    );
  }
