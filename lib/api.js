...
const { AppVersionManager, USER_AGENT } = require('./app-version');
...
class MercedesAPI {
  constructor(homey, oauth, region) {
    ...
    // One app identity for the whole client - REST, WebSocket and OAuth all
    // present the same version, and a refresh by any of them serves the rest.
    this.appVersion = new AppVersionManager(homey, region);
    if (oauth && typeof oauth.setAppVersionManager === 'function') {
      oauth.setAppVersionManager(this.appVersion);
    }
  ...
  async _getHeaders() {
    ...
    await this.appVersion.refresh(this.endpoints.rest);
    ...
    return this.appVersion.applyWebApiHeaders(headers);
  }
