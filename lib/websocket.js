...
class MercedesWebSocket {
  constructor(homey, oauth, region, protoParser) {
    ...
    this.RATE_LIMIT_BACKOFF = 600000; // 10 minutes - first window
    this.MAX_RATE_LIMIT_BACKOFF = 7200000; // 2 hours - cap
    ...
    for (const [vin, pidUpdates] of Object.entries(statusUpdates.updatesByVin)) {
      ...
      for (const status of Object.values(pidUpdates.updatesByPid)) {
        ...
        if (state === 'ACKED_BY_APPTWIN' || state === 'PIN_VALID') {
          ...
          // resolved at acceptance, so this reject() settles nothing. Log the
          // failure instead - it is the only account of what the vehicle
          // actually did with a command the user was told had been sent.
          if (!pending.awaitingCaller) {
            const first = (status.errors && status.errors[0]) || {};
            const why = COMMAND_ERROR_MESSAGES[first.code] || first.message || '';
            this.homey.app.error(
              `[WS] Command ${requestId} (${pending.commandName}) failed after its caller had already returned`
              + `: ${first.code || 'UNKNOWN'}${why ? ` - ${why}` : ''}`
            );
          }
   ...
   *
   * Each consecutive 429 doubles the window, up to MAX_RATE_LIMIT_BACKOFF.
  ...
  _registerRateLimit() {
    ...
    this.blockedFor = Math.min(
      this.RATE_LIMIT_BACKOFF * Math.pow(2, this.rateLimitStrikes - 1),
      this.MAX_RATE_LIMIT_BACKOFF
    );

    this.homey.app.error(
      `[WS] Account rate-limited (HTTP 429) - backing off for ${Math.round(this.blockedFor / 60000)} min `
