...
class MercedesWebSocket {
  ...
  _scheduleReconnect() {
    ...
    if (this.accountBlocked) {
      // The block is only worth honouring for as long as it plausibly
      // lasts; otherwise a single 429 pins every later reconnect to a
      // 10-minute delay until a connection happens to succeed.
      const blockedFor = this.blockedSinceTime ? Date.now() - this.blockedSinceTime : 0;
      if (blockedFor > 600000) {
  ...
  async sendCommand(message, requestId) {
    // One command at a time. Each command recycles the shared socket and
    // temporarily swaps `messageHandler`, so two overlapping commands would
    // tear down each other's session and cross their warmup state.
  ...
  /**
   * Run a single command on a freshly recycled session. Callers go through
   * sendCommand(), which serializes access.
   */
  async _sendCommandExclusive(message, requestId) {
    ...
    // Recycle via the recovery teardown, NOT disconnect(). disconnect() means
    // "the user asked us to stop": it sets isStopping, and the old code then
    // cleared the flag by hand. That left the previous socket abandoned with
    // its listeners still attached, so its late 'close' event fired against
    // the *new* socket — stopping its watchdogs (no more keepalive ping, no
    // more liveness detection) and scheduling a spurious reconnect that
    // swapped the socket mid-command. The result was a zombie socket that
    // still reported OPEN while receiving nothing, which is exactly how the
    // app "stalled" until it was restarted.
    this._teardownSocket('WebSocket session recycled for a new command');
    ...
    try {
      // connectOrThrow, not connect: a command must fail with the real
      // reason (401, 418, 429, handshake timeout, ...) instead of silently
      // continuing on a socket that never came up.
    ...
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      const detail = this._lastError ? this._lastError.message
        : (this._lastConnectError ? this._lastConnectError.message : 'connection dropped during session warmup');
      this._scheduleReconnect();
