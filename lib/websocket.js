...
class MercedesWebSocket {
  constructor(homey, oauth, region, protoParser, sessionId = null) {
    ...
    // ...and what to do when Mercedes says how long to wait instead.
    //
    // The ladder above is the app's own invention, kept for the case it was
    // written for: a 429 that carries no `Retry-After`. When the header is
    // there it replaces the guess outright - see resolveBackoff() for why the
    // ladder comes back as a floor once the header has been honoured and
    // refused through repeatedly.
    this.MIN_RETRY_AFTER = 5000; // floor, so `Retry-After: 0` is not a hot loop
    this.TRUST_RETRY_AFTER_STRIKES = 3; // consecutive refusals the header gets on its own
    ...
    // block is about the session rather than the token. Doors, windows, lock
    // state and sunroof exist only on that session (there is no REST source
    // for them - mbapi2020 polls the same two endpoints this app does), so
    // how fast an episode ends is the only thing that keeps them fresh.
    //
    // Measured on a live block: three refusals in a row, each one after a
    // successfully refreshed token, then one login and the socket opened in
    // 194 ms. So refreshing is close to worthless against this and the
    // escalation is deliberately early - the second refusal arms it, which
    // puts the first login about two minutes into an outage.
    //
    // The budget is three per episode, spent at most one per backoff window,
    // and a login that FAILS spends one attempt rather than ending the
    // recovery. It used to be one per episode outright, and the 12 Aug 2026
    // outage showed what that costs: the single login crashed (see
    // lib/oauth.js), no escalation remained, and the app sat refused for
    // five hours until the user restarted their Homey. mbapi2020 budgets
    // three the same way (MAX_RELOGIN_ATTEMPTS, incremented on failure).
    this._onRelogin = null;
    this._reloginAttempts = 0;
    this.MAX_RELOGIN_ATTEMPTS = 3;
    this.RELOGIN_AFTER_STRIKES = 2;
    ...
    // What the last refused upgrade actually said - status, headers and the
    // wait Mercedes asked for. Kept because for several releases the app knew
    // only the status code, scraped out of an error message string, and threw
    // every header away unread.
   ...
   *
   * The fallback for a failure that never produced a response object - the
   * status is scraped out of `ws`'s message string ("Unexpected server
   * response: 429"). When there *is* a response, _noteHandshakeRejection()
   * has already read it properly.
   */
  ...
  /**
   * Record what a refused upgrade actually said.
   *
   * The status code was all the app ever kept, and it came from an error
   * message rather than the response. Everything Mercedes sent alongside it
   * went unread - including `Retry-After`, the one number that can replace the
   * app's guessed backoff with the limiter's own answer. Rate-limit headers
   * are not secret and are the only evidence of what the limiter wants, so
   * they are logged whether or not this build knows what to do with them.
   *
   * @returns {{status: Number|null, headers: Object, retryAfterMs: Number|null}}
   */
  _noteHandshakeRejection(status, headers = {}) {
    const retryAfterMs = parseRetryAfter(headers['retry-after']);

    this.homey.app.log(
      `[WS] Handshake refused with HTTP ${status || 'unknown'} - `
      + `rate-limit headers: ${describeRateLimitHeaders(headers)} `
      + `(all headers: ${Object.keys(headers).join(', ') || 'none'})`,
    );

    this.lastHandshakeRejection = { status, headers, retryAfterMs, at: Date.now() };
    return this.lastHandshakeRejection;
  }
  ...
  async _connectInternal() {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        // Mercedes' answer to a refused upgrade, once it has been read off
        // the wire. Set by the 'unexpected-response' listener below, which is
        // the only place the response headers are reachable; everything after
        // it on this socket is teardown noise.
        let rejection = null;

        const handleFailure = (error, httpStatus) => {
          ...
          if (httpStatus === 429) {
            this._registerRateLimit(rejection ? rejection.retryAfterMs : null);
        ...
        // The refused handshake itself, headers and all.
        //
        // This is the only place `ws` exposes the upgrade response, and it
        // comes with a catch: `emit()` returns true when something is
        // listening, and that return value is exactly what `ws` uses to decide
        // whether to run its own abortHandshake(). Listening here means the
        // request is never torn down and the failure is never reported - so
        // the connect promise would never settle, `_connectPromise` would stay
        // set forever, and every later connect (reconnect timer, health check,
        // command) would await a dead promise. Both jobs are ours now.
        socket.on('unexpected-response', (req, res) => {
          const status = res && res.statusCode ? res.statusCode : null;
          rejection = this._noteHandshakeRejection(status, (res && res.headers) || {});

          try {
            // Drain the body, then hand the socket straight back to `ws`:
            // terminate() on a client that is still connecting runs the very
            // abortHandshake() that listening here suppressed. So the request
            // is destroyed and 'error'/'close' still arrive in the order the
            // rest of this class expects. Destroying the request by hand
            // instead leaves the client stuck in CONNECTING with no 'close'
            // ever emitted - which is how the socket ends up abandoned rather
            // than released.
            if (res && typeof res.resume === 'function') res.resume();
            socket.terminate();
          } catch (teardownError) {
            this.homey.app.log(`[WS] Error discarding refused handshake: ${teardownError.message}`);
          }

          // Report it exactly as `ws` would have. Its abort announces itself a
          // tick later as "WebSocket was closed before the connection was
          // established", carrying no status at all, so leaving the reporting
          // to it would turn a 429 into a transport blip - no backoff window,
          // no token refresh, no re-login.
          handleFailure(new Error(`Unexpected server response: ${status}`), status);
        });

        // Connection error (fires before 'close' on handshake failure)
        socket.on('error', (error) => {
          if (rejection) {
            // Already reported above, with far more detail than this carries.
            // Letting it through would overwrite `_lastError` - and with it
            // the reason a command failed and the close handler's account of
            // what happened - with "socket hang up".
            this.homey.app.log(`[WS] Handshake teardown after HTTP ${rejection.status}: ${error.message}`);
            settleErr(error);
            return;
          }

          handleFailure(error, this._extractHttpStatus(error));
        });
   ...
   *
   * If Mercedes said how long to wait, that is the window: the app has no
   * business overruling the limiter's own answer, and the overwhelmingly
   * common 429 - the one every app restart earns - clears in seconds.
   *
   * Without a `Retry-After` the app falls back to its own ladder, where each
   * consecutive 429 widens the window by RATE_LIMIT_BACKOFF_FACTOR up to
   * MAX_RATE_LIMIT_BACKOFF. Retrying at a fixed interval is what kept an
   * account blocked indefinitely: Mercedes wants a quiet period, and a window
   * that never grows never provides one. The ladder keeps escalating in the
   * background even while the header is being honoured, so it is ready the
   * moment the header stops being believable (see resolveBackoff).
   *
   * @param {Number|null} [retryAfterMs] - what the response asked for, if it did
   */
  _registerRateLimit(retryAfterMs = null) {
    ...
    const { ms, askedMs } = resolveBackoff({
      retryAfterMs,
      fallbackMs: this.RATE_LIMIT_BACKOFF
        * Math.pow(this.RATE_LIMIT_BACKOFF_FACTOR, this.rateLimitStrikes - 1),
      minMs: this.MIN_RETRY_AFTER,
      maxMs: this.MAX_RATE_LIMIT_BACKOFF,
      trusted: this.rateLimitStrikes < this.TRUST_RETRY_AFTER_STRIKES,
    });
    this.blockedFor = ms;
    ...
    this.homey.app.error(
      ...
      + `(${source}; consecutive rate-limited attempts: ${this.rateLimitStrikes})`
    );
  }
