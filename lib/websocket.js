...
class MercedesWebSocket {
  constructor(homey, oauth, region, protoParser, appVersion = null) {
    ...
    // Commands the Flow has stopped waiting for, but the vehicle has not
    // finished. Their outcome lands after the Flow has been answered, so it is
    // tracked here to be reported rather than dropped.
    // requestId -> { commandName }
    this._awaitingOutcome = new Map();

    // Called when a command fails at the vehicle after its Flow already
    // returned. The Flow card cannot be un-answered, so this is the only way
    // the user can find out - see setCommandFailedHandler().
    ...
    // How long a Flow action waits for Mercedes to accept the command.
    //
    // Waiting for the vehicle to actually finish would be the honest answer,
    // and it was tried. It does not survive contact with the transport: the
    // status stream stalls. One captured windowsOpen was acked in 160ms, then
    // the socket delivered nothing at all - no statuses, no vepUpdates - for
    // 34.5s, flushed the backlog in a burst (six duplicate FINISHED frames in
    // 130ms), and dropped with a 1006 abnormal closure 83s later. The windows
    // had opened the whole time; only the telling was late. A Flow card that
    // waits for completion fails every time that happens.
    //
    // Acceptance does not have that problem - every acceptance measured
    // landed in 59-290ms, including the one during that stall.
    //
    // What acceptance does NOT mean is that the car acted: a windowsClose
    // once acked in 81ms, sat in Mercedes' queue for eight and a half
    // minutes, and came back FAILED/CMD_TIMEOUT. That outcome is followed
    // past the Flow's return and surfaced through the command_failed trigger
    // - see _awaitingOutcome.
    this.COMMAND_ACCEPTANCE_TIMEOUT = 8000; // 8 seconds
  ...
  _teardownSocket(reason = 'WebSocket connection lost while command was pending') {
    ...
    // Status updates are scoped to a session, so nothing from the old one can
    // still arrive. Clearing here is also what keeps the set from growing for
    // the lifetime of the app.
    this._awaitingOutcome.clear();
  ...
  /**
   * Report a late command failure, without letting a handler fault take the
   * WebSocket down with it - this runs inside the message loop.
   */
  ...
  /**
   * Register a callback for commands that fail after their Flow has returned.
   *
   * Mercedes reports completion ~12s after acceptance, which is past the point
   * Homey abandons a Flow card - so the card has already said "success" by the
   * time a failure is known. Without this the failure only ever reached the
   * log.
   *
  ...
  _handleCommandStatusUpdates(statusUpdates) {
    ...
    for (const [vin, pidUpdates] of Object.entries(statusUpdates.updatesByVin)) {
      ...
      for (const status of Object.values(pidUpdates.updatesByPid)) {
        ...
        if (!pending) {
          // The Flow has already been answered - on acceptance, or because it
          // gave up waiting. What the vehicle then did still matters, and
          // these lines are the only place it is reported.
          if (this._awaitingOutcome.has(requestId) && (state === 'FINISHED' || state === 'FAILED')) {
            const { commandName } = this._awaitingOutcome.get(requestId);
            this._awaitingOutcome.delete(requestId);

            if (state === 'FAILED') {
              this.homey.app.error(`[WS] Command ${requestId} (${commandName}) FAILED at the vehicle, after the Flow had already returned`);
              if (status.errors && status.errors.length > 0) {
                this.homey.app.error(`[WS] Late command errors:`, JSON.stringify(status.errors));
              }
              ...
              this.homey.app.log(`[WS] Command ${requestId} (${commandName}) confirmed complete at the vehicle`);
            }
          }
          continue;
        }

        // Resolve on acceptance. Completion is the answer the user wants, but
        // it rides a stream that can stall for 30s+ (see
        // COMMAND_ACCEPTANCE_TIMEOUT), so waiting for it makes the Flow fail
        // on transport hiccups for commands the car carried out fine.
        // `confirmed` distinguishes the two for callers that care.
        if (state === 'ACKED_BY_APPTWIN' || state === 'PIN_VALID' || state === 'FINISHED') {
          clearTimeout(pending.timeout);
          this.homey.app.log(`[WS] Command ${requestId} accepted (${state})`);
          pending.resolve({ success: true, state, confirmed: state === 'FINISHED' });
          this.pendingCommands.delete(requestId);

          // Accepted is not done. Keep watching so the vehicle's real answer
          // reaches the command_failed trigger instead of vanishing.
          if (state !== 'FINISHED') {
            this._awaitingOutcome.set(requestId, { commandName: pending.commandName });
          }
          ...
          // INITIATION, ENQUEUED, PROCESSING, WAITING, the wakeup states: all
          // progress, none of them an outcome. Keep waiting - the acceptance
          // timeout stays armed.
          this.homey.app.log(`[WS] Command ${requestId} progressed to ${state}, still waiting for completion`);
        }
      }
    }
  }
   ...
   * @returns {Promise<{success: true, state: string, confirmed: boolean}>}
   *   Resolves when Mercedes accepts the command - `confirmed` is true only if
   *   the vehicle had already reported it finished. Rejects on FAILED, on an
   *   unusable connection, or when nothing arrives within
   *   COMMAND_ACCEPTANCE_TIMEOUT. Either way the real outcome is followed up
   *   and surfaced through the command_failed trigger
   */
  ...
  async _sendCommandExclusive(message, requestId, commandName = 'unknown') {
    ...
    // Wait for Mercedes to accept the command (or refuse it outright)
    const commandPromise = new Promise((resolve, reject) => {
      const timeoutMs = this.COMMAND_ACCEPTANCE_TIMEOUT;
      const timeout = setTimeout(() => {
        this.pendingCommands.delete(requestId);

        // Mercedes may still answer long after this. Remember the id so the
        // outcome is reported - via the command_failed trigger if it failed -
        // rather than dropped as a status for a request nothing recognises.
        this._awaitingOutcome.set(requestId, { commandName });

        reject(new Error(
          `Command not acknowledged within ${Math.round(timeoutMs / 1000)}s `
          + '(no response from Mercedes - it may still be carried out)',
        ));
      }, timeoutMs);

      this.pendingCommands.set(requestId, { resolve, reject, timeout, commandName });
    });
